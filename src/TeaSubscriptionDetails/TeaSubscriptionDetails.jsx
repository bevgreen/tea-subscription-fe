...
//     customerNames.push(`${customer?.attributes?.first_name} ${customer?.attributes?.last_name}`)

//     const customerSubscription = subDetails?.included?.find(
//       (item) => item.type === 'customer_subscription' && item.relationships?.customer?.data?.id === customer.id
//     )

//     customerStatuses.push(customerSubscription?.attributes?.status || 'inactive')
//   })

//   function toggleSubscriptionStatus(customerId, currentStatus) {
//     let newStatus
...
//       newStatus = 'active'
//     }
//     fetch(`http://localhost:3000/api/v1/customer_subscriptions/${customerId}`, {
//       method: 'PATCH', 
//       headers: {
...
//         // need to go through subDetails and find the customer subscription that just updated
//         // update that subs status to new value
//         const updatedCustomers = subDetails.included.map((item) => {
//           if (item.type === 'customer_subscription' && item.relationships.customer.data.id === customerId) {
//             return {
//               ...item, // keep all existing properties 
//               attributes: {
//                 ...item.attributes, // keep the existing attributes
//                 status: newStatus, // change only the status 
//               },
//             }
//           }
...
//         })
//         setSubDetails({
//           ...subDetails, // keep the rest of the data 
//           included: updatedCustomers, // update the list of customer subscriptions with the updated status
//         })
//       })
//       .catch((err) => {
...
//               <button onClick={() => toggleSubscriptionStatus(customer.id, customerStatuses[index])}>
//                 {customerStatuses[index] === 'active' ? 'Deactivate' : 'Activate'}
//               </button>
//             </li>
//           ))
//         ) : (
